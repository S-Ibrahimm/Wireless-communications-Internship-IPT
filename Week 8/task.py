...
# [NEW WEEK 8] RIS phase configuration choices (0: Unaligned/Random, 1: Perfect Alignment with N^2 gain)
    ...
    # [NEW WEEK 8] Joint action space expanded to (Power, Modulation, RIS Phase Shift)
        ...
        high = np.array([500.0, -30.0], dtype=np.float32)
    ...
    def _get_normalized_state(self):
        ...
        norm_gain = (self.chan_gain_db - (-120.0)) / (-30.0 - (-120.0))
    ...
    def _compute_channel_gain_db(self, distance, ris_mode=0):
        # Path loss reference at 1m (38.5dB) for realistic signal strength scale
        pl_0_db = 38.5

        # 1. Direct Link Channel Gain (Base Station -> User)
        path_loss_direct_db = pl_0_db + (10.0 * self.eta * np.log10(distance))
        fading_linear = np.random.exponential(1.0)
        gain_direct_linear = (10.0 ** (-path_loss_direct_db / 10.0)) * max(
            fading_linear, 1e-4
        )


        # 2. Reflected Link Channel Gain (BS -> RIS -> User)
        d_ris_ue = max(abs(distance - self.d_bs_ris), 1.0)
        path_loss_bs_ris_db = pl_0_db + (10.0 * self.eta_ris * np.log10(self.d_bs_ris))
        path_loss_ris_ue_db = pl_0_db + (10.0 * self.eta_ris * np.log10(d_ris_ue))
        total_ris_path_loss_db = path_loss_bs_ris_db + path_loss_ris_ue_db
    ...
    def reset(self, seed=None, options=None):
        ...
        # Randomize initial distance between 10m and 500m
...
if __name__ == "__main__":  # boilerplate
    ...
    num_episodes = 2000
    batch_size = 64 #NN will sample and learn from 64 random past experiences at each training step,
    target_update_freq = 10 #weights are refreshed every 10 episodes to prevent training divergence.
