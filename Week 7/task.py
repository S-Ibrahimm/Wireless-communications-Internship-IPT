...
class WirelessEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_list = [(p,m) for  p in self.power_lvls for m in self.modulations]
    ...
    def reset(self, seed=None, options=None):
        ...
        initial_p_tx_dbm, _ = self.action_list[0]
    ...
    def step(self, action):
        self.current_step += 1
        #[UPDATED WEEK 7] Unpack joint action into transmit power and modulation scheme
        p_tx_dbm, modulation = self.action_list[action]
        ...
        # [NEW WEEK 7] Calculate BER and Throughput for joint reward calculation
        ber = self._compute_ber(self.snr_db, modulation)
        data_rate = self._compute_data_rate(modulation)

        #Week 7: find BER throughput for joint reward calculation
        if ber > 0.001:
            reward = -10.0 -(self.lam * max(0, p_tx_dbm))
        else:
            reward = data_rate - (self.lam * max(0, p_tx_dbm))
...
def baseline_fixed(env):
    """Baseline 2: Fixed heuristic that always uses Max Power (20 dBm) and QPSK."""
    target_tuple = (20, "QPSK")
    return env.action_list.index(target_tuple)


def baseline_greedy(env):
    """Baseline 3: Traditional Adaptive Modulation & Coding (AMC) rule based on current SNR."""
    if env.snr_db >= 15.0:
        target_tuple = (5, "16-QAM")  # High SNR -> High modulation, moderate power
    elif env.snr_db >= 8.0:
        target_tuple = (10, "QPSK")  # Medium SNR -> Moderate modulation, higher power
    else:
        target_tuple = (20, "BPSK")  # Low SNR -> Robust modulation, max power
...
if __name__ == "__main__":  # boilerplate
    ...
    for step in range(1, 6):
        ...
        chosen_power, chosen_mod = env.action_list[dqn_action]

        eval_state, reward, _, _, _ = env.step(dqn_action)
        ber = env._compute_ber(env.snr_db, chosen_mod)

        # 2. Greedy Baseline Action for comparison
        greedy_act = baseline_greedy(env)
        g_power, g_mod = env.action_list[greedy_act]

        print(
            f"Step {step} | Dist: {env.distance:.1f}m | "
            f"DQN Choice: [{chosen_power:2d} dBm, {chosen_mod:6s}] -> SNR: {env.snr_db:.1f}dB, BER: {ber:.4f}, Reward: {reward:.2f} | "
            f"Greedy Rule: [{g_power:2d} dBm, {g_mod:6s}]"
        )